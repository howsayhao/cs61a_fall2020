...
def prune_min(t):
    ...
    if len(t.branches) == 2:
        l = t.branches
        t.branches = [l[0]] if l[0].label < l[1].label else [l[1]]
...
def num_splits(s, d):
    ...
    max = (sum + d)//2
    # print(min, max)
    def mutate_num_splits(curr, s):
        nonlocal num
        if not s:
            return;
        now = curr + s[0]
        if now >= min and now <= max:
            num = num + 1
        mutate_num_splits(curr+s[0], s[1:])
        mutate_num_splits(curr, s[1:])
    mutate_num_splits(0, s)
    num = num//2
    if num_sum(s) >= min and num_sum(s) <= max:
        num = num + 1
    return num
