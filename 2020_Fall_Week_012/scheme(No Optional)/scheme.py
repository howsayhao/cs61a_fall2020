...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        return scheme_apply(operator, rest.map(lambda z: scheme_eval(z, env)), env)
        # END PROBLEM 4
...
def eval_all(expressions, env):
    ...
    if expressions is nil:
        ...
        # 这里一定得用map就很烦，这里各个class分的太细，很容易记不清对应的处理函数是啥
        rst = expressions.map((lambda z: scheme_eval(z, env)))
        while rst.rest is not nil:
            rst = rst.rest
        return rst.first
    # END PROBLEM 7
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        it, that = formals, vals
        while it is not nil and that is not nil:
            childframe.define(it.first, that.first)
            it, that = it.rest, that.rest
        return childframe
        # END PROBLEM 10
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 11
        childframe = self.env.make_child_frame(self.formals, args)
        return childframe
        # END PROBLEM 11
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        it = target.rest
        while it is not nil:
            if not scheme_symbolp(it.first):
                raise SchemeError('炸了')
            it = it.rest
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 8
    # 我是真无语了，之前一直想用pair来写
    # 那样只是一个pair链，而不是一个LambdaProcedure对象
    return LambdaProcedure(formals, expressions.rest, env)
    # return Pair('lambda', Pair(formals, expressions.rest))
    # END PROBLEM 8
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        return True
    # 简直逆天，xxx.scheme_eval()和scheme_eval(xxx)是不一样的
    # 这俩取一样的名字...
    it = expressions
    while it is not nil:
        that = scheme_eval(it.first, env)
        if is_false_primitive(that):
            return False
        if  it.rest is nil:
            return that
        it = it.rest
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    bindings.map(lambda z : validate_form(z, 2, 2))
    
    names = bindings.map(lambda z : z.first)
    values = bindings.map(lambda z : scheme_eval(z.rest.first, env))
...
class MuProcedure(Procedure):
    ...
    def __init__(self, formals, body):
        """A procedure with formal parameter listde  FORMALS (a Scheme list) and
        Scheme list BODY as its definition."""
        self.formals = formals
        self.body = body

    # BEGIN PROBLEM 18
    # 无语，他这个是不是没讲清楚，面向测试编程...
    def make_call_frame(self, args, env):
