...
def flatten(lst):
    ...
    for dex in range(len(lst)):
        if( type(lst[dex]) == list ):
                lest += flatten(lst[dex])
        else:
            lest += [lst[dex]]
    return lest
...
def merge(lst1, lst2):
    ...
    if( lst1 == [] ):
        ...
        return lst1
    for dex in range(len(lst2)):
        for index in range(len(lst1)):
            if(index == 0):
                if( lst2[dex] < lst1[index] ):
                    lst1 = [lst2[dex]] + lst1[:]
                    break
            elif (lst2[dex] < lst1[index]) and (lst2[dex] > lst1[index-1]):
                lst1 = lst1[:index] + [lst2[dex]] + lst1[index:]
                break
            else:
                if( index == len(lst1)-1 ):
                    lst1 = lst1[:] + [lst2[dex]]
...
def check_win(board, max_rows, max_cols, num_connect, row, col, player):
    ...
    if( check_win_row(board,max_rows,max_cols,num_connect,row,player) ):
        return True
    elif( check_win_column(board,max_rows,max_cols,num_connect,row,player) ):
        return True
    elif( diagonal_win ):
        return True
    else:
