...
class Insect(object):
    ...
    def __init__(self, armor, place=None):
        ...
        self.armor = armor
        self.place = place  # set by Place.add_inscte and Place.remove_insect
...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        colony -- The AntColony, used to access game state information.
        """
        if self.stun_duration or self.slow_duration:
            if self.stun_duration:
                self.stun_duration -= 1
                if self.slow_duration:
                    self.slow_duration -= 1
            elif self.slow_duration:
                self.slow_duration -= 1
                if colony.time%2 == 0:
                    if self.blocked():
...
class Ant(Insect):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        if self.container == True and self.ant == None:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        range = 0
        while Place != hive:
            if Place.bees != [] and range >= self.min_range and range <= self.max_range:
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        # BEGIN Problem 11
        "*** YOUR CODE HERE ***"
        water_place = Place(self.name)
        water_place.bees = self.bees
        water_place.ant = self.ant 
        water_place.add_insect(insect)
        if insect.watersafe == False:
            insect.reduce_armor(insect.armor)
        self.ant = water_place.ant
        self.bees = water_place.bees
        insect.place = self
        ...
        if self.armor <= 0:
            for x in self.place.bees:
                if x.armor > self.damage:
                    x.armor = x.armor - self.damage
                    exists = exists + [x]
            self.place.bees = exists
            self.place.ant = None
        # END Problem 5
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt can do nothing but with high armor"""
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.true_Queen == False:
            ...
            while current != None:
                if current.ant != None:
                    if current.ant not in self.inspired:
                        current.ant.damage = current.ant.damage * 2
                        self.inspired.append(current.ant)
                    if current.ant.container == True and current.ant.ant != None:
                        if current.ant.ant not in self.inspired:
                            current.ant.ant.damage = current.ant.ant.damage * 2
                            self.inspired.append(current.ant.ant)
