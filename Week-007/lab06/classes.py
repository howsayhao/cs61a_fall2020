    ...
    def take(self, thing):
        ...
        if type(thing) != str:
            ...
            if thing in self.place.things.keys():
                ...
                self.backpack = self.backpack + [self.place.things[thing]]
...
""" Implement Key here! """
class Key(Thing):
    def use(self,place,Place):
        if Place.exits[place][0].locked == True:
            print(place,'is now unlocked!')
            Place.exits[place][0].locked = False
        else:
