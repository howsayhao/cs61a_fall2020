...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        def transform(x, env):
            if type(x) == Name:
                if type(env[x.string]) == Number:
                    return env[x.string]
                elif type(env[x.string]) == Literal:
                    return Number(env[x.string].value)
            elif type(x) == Literal:
        ...
        if type(self.operator) == Name:
            op = [transform(x, env) for x in self.operands]
            return env[self.operator.string].apply(op)
        elif type(self.operator) == CallExpr:
            arg = []
            x = self
            while type(x.operator) == CallExpr:
                arg += x.operands
                x = x.operator
            arg += x.operands
            x = x.operator
            paramet = []
            while type(x.body) == LambdaExpr:
                paramet += x.parameters
                x = x.body
            paramet += x.parameters
            new_env = env.copy()
            new_env.update(dict(zip(paramet ,arg)))
            return x.body.eval(new_env)
        elif type(self.operator) == LambdaExpr:
            arg = [transform(x,env) for x in self.operands]
            x = self.operator
            paramet = []
            while type(x.body) == LambdaExpr:
                paramet += x.parameters
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        if len(self.parameters) != len(arguments):
            ...
            if type(self.body) == CallExpr:
                env.update(dict(zip(self.parameters , arguments)))
                return self.body.eval(env)
            else:
                if type(self.body) == Literal:
                    return Number(self.body.value)
                elif type(self.body) == Number:
                    return self.body
